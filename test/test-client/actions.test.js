...
describe('sync actions', () => {
	...
	it('should show overlay', () => {
		...
		expect(overlayActions.showOverlay(enlargedState)).toEqual(
			expectedAction
		);
	});
...
describe('async actions', () => {
	...
	it('creates FETCH_CATEGORY_STATE_SUCCESS when fetching state data has succeeded', () => {
		...
		return store
			.dispatch(handleDataActions.fetchCategoryState('age'))
	...
	it('creates FETCH_CATEGORY_STATE_ERROR when fetching state data has failed', () => {
		...
		const expectedActions = [
			{
				...
				error: new Error(
					'invalid json response body at /api/INVALID/state reason: ' +
		...
		return store
			.dispatch(handleDataActions.fetchCategoryState('INVALID'))
			.then(() => expect(store.getActions()).toEqual(expectedActions));
	});
	...
	it('creates FETCH_CATEGORY_STATE_SUCCESS when fetching state data has succeeded', () => {
		...
		return store
			.dispatch(handleDataActions.fetchCategoryCounty('age', '01'))
			.then(() => expect(store.getActions()).toEqual(expectedActions));
	});
	...
	it('creates FETCH_CATEGORY_COUNTY_ERROR when fetching state data has failed', () => {
		...
		const expectedActions = [
			{
				...
				error: new Error(
					'invalid json response body at /api/INVALID/county/01 reason: ' +
						'Unexpected end of JSON input'
				)
			}
		];
		...
		return store
			.dispatch(handleDataActions.fetchCategoryCounty('INVALID', '01'))
			.then(() => expect(store.getActions()).toEqual(expectedActions));
	});
});
